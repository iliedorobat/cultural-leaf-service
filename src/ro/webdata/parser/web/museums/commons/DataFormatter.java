...
public class DataFormatter {
    ...
    private static final List NAME_FORMAT_LIST = Arrays.asList(
            BackendAccessors.CONTACT_DIRECTOR,
            BackendAccessors.CONTACT_PERSON_NAME,
            BackendAccessors.CONTACT_PERSON_POSITION,
            BackendAccessors.LOCATION_GEO_TARGET,
            BackendAccessors.LOCATION_ADM_UNIT,
            BackendAccessors.LOCATION_COMMUNE,
            BackendAccessors.LOCATION_COUNTY,
            BackendAccessors.LOCATION_LOCALITY_NAME,
            BackendAccessors.MUSEUM_NAME,
            BackendAccessors.MUSEUM_SUBORDINATION_NAME
    );
    private static final List CATEGORY_LIST = Arrays.asList(
            BackendAccessors.COLLECTION_CATEGORY,
            BackendAccessors.COLLECTION_GENERAL_PROFILE,
            BackendAccessors.COLLECTION_MAIN_PROFILE
    );
    private static final List GEO_LIST = Arrays.asList(
            BackendAccessors.LOCATION_GEO_LATITUDE,
            BackendAccessors.LOCATION_GEO_LONGITUDE
    );
    private static final List PHONE_LIST = Arrays.asList(
            BackendAccessors.CONTACT_FAX,
            BackendAccessors.CONTACT_PHONE
    );

    public static String format(String key, String value) {
        String formatted = formatDiacritics(value);

        if (NAME_FORMAT_LIST.indexOf(key) != -1)
            return formatName(formatted);
        else if (CATEGORY_LIST.indexOf(key) != -1)
            return formatCategory(formatted);
        else if (PHONE_LIST.indexOf(key) != -1)
            return formatPhone(formatted);
        else if (GEO_LIST.indexOf(key) != -1)
            return formatGeoCoord(formatted);
     ...
     */
    private static String formatDiacritics(String str) {
